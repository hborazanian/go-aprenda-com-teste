...
func TestPercorre(t *testing.T) {
	t.Run("sem maps", func(t *testing.T) {
		casos := []struct {
			Nome              string
			Entrada           interface{}
			ChamadasEsperadas []string
		}{
			{"Struct com um campo string", struct{ Nome string }{"Chris"}, []string{"Chris"}},
			{
			...
			{
				"Slices",
				[]Perfil{{33, "Londres"}, {34, "Reykjavík"}},
				[]string{"Londres", "Reykjavík"},
			},
			{
				"Arrays",
				[2]Perfil{{33, "Londres"}, {34, "Reykjavík"}},
				[]string{"Londres", "Reykjavík"},
			},
		}
